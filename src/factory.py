...
from .riscv import get_spec as get_rv_spec, riscv_fuzzer, riscv_generator

GENERATORS: Dict[str, Type[interfaces.Generator]] = {
    "x86-64-random": x86_generator.X86RandomGenerator,
    "riscv-random": riscv_generator.RISCVRandomGenerator,
}
...
def get_fuzzer(instruction_set, working_directory, testcase, inputs):
    if CONF.fuzzer == "architectural":
        if CONF.instruction_set == "x86-64":
            return x86_fuzzer.X86ArchitecturalFuzzer(
                instruction_set, working_directory, testcase, inputs
            )
        raise ConfigException(
            "ERROR: unknown value of `instruction_set` configuration option"
        )
    elif CONF.fuzzer == "basic":
        if CONF.instruction_set == "x86-64":
            return x86_fuzzer.X86Fuzzer(
                instruction_set, working_directory, testcase, inputs
            )
        elif CONF.instruction_set == "riscv":
            return riscv_fuzzer.RISCVFuzzer(
                instruction_set, working_directory, testcase, inputs
            )
        raise ConfigException(
            "ERROR: unknown value of `instruction_set` configuration option"
...
def get_program_generator(
    instruction_set: interfaces.InstructionSetAbstract, seed: int
) -> interfaces.Generator:
    return _get_from_config(
        GENERATORS,
        CONF.instruction_set + "-" + CONF.generator,
        "instruction_set",
        instruction_set,
        seed,
    )
...
def get_input_generator(seed: int) -> interfaces.InputGenerator:
    return _get_from_config(
        INPUT_GENERATORS, CONF.input_generator, "input_generator", seed
    )
...
def get_model(bases: Tuple[int, int]) -> interfaces.Model:
    ...
    if CONF.instruction_set == "x86-64":
        if (
            "cond" in CONF.contract_execution_clause
            ...
            model_instance = _get_from_config(
                X86_SIMPLE_EXECUTION_CLAUSES,
    ...
    model_instance.tracer = _get_from_config(
        TRACERS, CONF.contract_observation_clause, "contract_observation_clause"
    )
...
def get_coverage(
    instruction_set: interfaces.InstructionSetAbstract,
    executor_: interfaces.Executor,
    model: interfaces.Model,
    analyser: interfaces.Analyser,
) -> interfaces.Coverage:
    return _get_from_config(
        COVERAGE,
        CONF.coverage_type,
...
def get_downloader(arch: str, extensions: List[str], out_file: str) -> Callable:
    return _get_from_config(
        SPEC_DOWNLOADERS, arch, "architecture", extensions, out_file
    )
